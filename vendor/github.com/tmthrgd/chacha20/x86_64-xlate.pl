...
#
# All modifications are copyright 2016 Tom Thorogood. All rights
# reserved. All modifications are governed by a Modified BSD License
...
my $win64=0;
my $golang=0; my $golang_no_avx=0;
my $prefix="";
my $decor=".L";
...
if ($flavour eq "golang") { $gas=0; $elf=0; $golang=1; }
elsif ($flavour eq "golang-no-avx") { $gas=0; $elf=0; $golang=1; $golang_no_avx=1; }
elsif ($flavour eq "mingw64")	{ $gas=1; $elf=0; $win64=1;
...
{ package opcode;	# pick up opcodes
	...
	if ($gas) {
	    ...
	    if ($self->{op} eq "vpaddd16" || $self->{op} eq "vpaddd0") {
                $self->{op} = "vpaddd";
            }
...
{ package label;	# pick up labels, which end with :
	...
	if ($golang && $self->{value} =~ /^[0-9]$/) {
		if (exists($golang_last_label_id{$self->{value}})) {
			$golang_last_label_id{$self->{value}} = chr(ord($golang_last_label_id{$self->{value}}) + 1);
		} else {
			$golang_last_label_id{$self->{value}} = "a";
		}

		$self->{value} = "label$self->{value}$golang_last_label_id{$self->{value}}";
		$ret = $self;
	}
...
{ package expr;		# pick up expressioins
	...
	if ($nasm && opcode->mnemonic()=~m/^j/) {
	    ...
	    if ($self->{value} =~ /^([0-9]+)([bf])$/) {
		if ($2 eq "b") {
			"label$1$golang_last_label_id{$1}";
		} else {
			my $id = "a";

			if (exists($golang_last_label_id{$1})) {
				$id = chr(ord($golang_last_label_id{$1}) + 1);
			}

			"label$1$id";
		}
	    } elsif ($self->{value} =~ /([0-9]+\*[0-9]+)\(/) {
	        my $val = $1;
...
{ package directive;	# pick up directives, which start with .
	...
	if ($line =~ /^\s*(\.\w+)/) {
	    ...
	    $line = "";
	} elsif ($golang && $line =~ /^\s*(TEXT|DATA|GLOBL)\s/) {
	    $self->{value} = $line;
	    $line = "";
	    return $self;
	}
...
while($line=<>) {
	...
	if ($#args>=0) {
		...
		if ($golang_no_avx && $argline !~ /\((?:SP|FP|SB)\)/ && grep { $insn eq $_ } @golang_missing_avx) {
		    ...
		    open my $ash, '|-', 'as', '-o', $filename, '-' or die "Can't open pipe: $!";
		    
		    $gas=1; $elf=1; $golang=0;
		    printf $ash "%s\t%s\n",$opcode->out($sz),join(",",map($_->out($sz),@args));
		    $gas=0; $elf=0; $golang=1;

		    close $ash or die "Can't close pipe: $!";
		    
		    open my $objh, '-|', 'objdump', '-d', $filename or die "Can't open pipe: $!";
		    ...
		    close $objh or die "Can't close pipe: $!";
